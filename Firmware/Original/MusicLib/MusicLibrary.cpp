...
{ 
	...
	for (int i = 0; i < 80; i++)
	{
		for (int i = 0; i < 55; i++) digitalWrite(Pinout[i].Step, LOW);
		delay(2);
		for (int i = 0; i < 5; i++) digitalWrite(Pinout[i].Step, HIGH);
		delay(2);
	}
	for (int i = 0; i < 5; i++) digitalWrite(Pinout[i].Dir, LOW);
	for (int i = 0; i < 30; i++)
	{
		for (int i = 0; i < 5; i++) digitalWrite(Pinout[i].Step, HIGH);
		delay(2);
		for (int i = 0; i < 5; i++) digitalWrite(Pinout[i].Step, LOW);
		delay(2);
	}
	for (int i = 0; i < 5; i++) digitalWrite(Pinout[i].DriveSelect, HIGH);
}
void PlaySong(Song song, int noteOffsetLow, int noteOffsetHigh, float speedModifier, float noteCutoff)
{
	...
	int buttonCheckCountDown = 1000;

	while (true)
	{
		if (channels[0].IsDone && channels[1].IsDone && channels[2].IsDone && channels[3].IsDone && channels[4].IsDone && channels[5].IsDone) break;
		if (--buttonCheckCountDown <= 0)
		{
			buttonCheckCountDown = 1000;
		...
		for (int i = 0; i < 5; i++)
		{
			...
			if (!channel->IsPlaying && !channel->IsDone)
			{
				...
				if (channel->IsPausing)
				{
					...
					int note = GetSongElement(song, channel->CurrentPosition + 5);
					channel->CurrentPosition += 7;
